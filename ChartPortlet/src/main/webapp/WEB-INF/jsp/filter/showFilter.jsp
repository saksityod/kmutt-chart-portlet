...
<body>
	<form:form id="globalFilterForm" modelAttribute="globalFilterForm"
		method="post" name="globalFilterForm" action="${formAction}"
		enctype="multipart/form-data" style="margin:0 0 0 0">
		<img id="cascadeWaiting"
		...
		<c:if test="${not empty globalFilterForm.filterList}">
			<c:forEach items="${globalFilterForm.filterList}" var="filter"
				varStatus="loop">
				<div style="display: inline-block">
					...
					<c:choose>
						<c:when test="${filter.valueType.equals('manual input') }">
							<input type="text"
								id="g_filter_${filter.connId}_${filter.filterId}"
								name="g_filter_${filter.connId}_${filter.filterId}"
								class="global_filter" value="${filter.selectedValue}"
								data-filter="${filter.filterName}"
								onchange="cascadeGlobal(this)" />
						</c:when>
						<c:when test="${filter.valueType.equals('select')}">
							<select id="g_filter_${filter.connId}_${filter.filterId}"
								name="g_filter_${filter.connId}_${filter.filterId}"
								class="global_filter" data-filter="${filter.filterName}"
								onchange="cascadeGlobal(this)">
		...
		<div
			style="display: inline-block; padding: 6px 1px 1px 5px; float: right;">
			<a tabindex="0" id="${ns}comment_bt" data-toggle="popover"
				data-trigger="focus" title="Comment"
				data-content="${globalFilterForm.comment}"> <img
				id="${ns}linktox"
